...
try:
    ...
    LLM_INVOKE_LATENCY = Histogram('llm_invoke_latency_seconds', 'LLM invoke latency seconds', buckets=[0.01,0.05,0.1,0.2,0.5,1,2,5,10])
    LLM_TOKENS_GENERATED = Counter('llm_tokens_generated_total', 'Estimated tokens generated by LLM')
    LLM_INVOKE_ERRORS = Counter('llm_invoke_errors_total', 'LLM invoke errors')

    CLASSIFICATION_PROCESSED = Counter('classification_processed_total', 'Number of texts processed')
    CLASSIFICATION_ERRORS = Counter('classification_errors_total', 'Number of processing errors')

    PROC_CPU = Gauge('process_cpu_percent', 'Process CPU percent')
    PROC_MEM_RSS = Gauge('process_resident_memory_bytes', 'Process resident memory bytes')
    GPU_UTIL = Gauge('gpu_util_percent', 'GPU utilization percent', ['gpu_index'])
except Exception:
...
CLASSIFY_USE_CHECKPOINT = os.getenv('CLASSIFY_USE_CHECKPOINT', '1') == '1'
...
class UniversalPromptOptimizer:
    def __init__(self):
        ...
        self.model_profiles = {
            ...
            'komodo_chat': {'optimal_context': 4096, 'preferred_format': 'chatml'},
            'komodo': {'optimal_context': 4096, 'preferred_format': 'mistral'},
            
            # Phi series
            'phi': {'optimal_context': 4096, 'preferred_format': 'chatml'},
    ...
    def detect_model_family(self, model_name):
        ...
        # jan-hq/komodo-7b-chat and similar chat checkpoints use ChatML wrapper.
        if 'komodo' in model_lower and 'chat' in model_lower:
            return 'komodo_chat'
    ...
    def build_model_specific_prompt(self, components, text, model_name, max_tokens):
        ...
        templates = {
            ...
            'komodo_chat': self.build_chatml_prompt,
    ...
    def build_chatml_prompt(self, components, text, max_tokens):
        ...
        if 'base' in components:
            system_content += self.compress_instructions(components['base'].content.replace('{text}', text))
    ...
    def build_aggressive_prompt(self, text, model_family):
        ...
        elif model_family in ['komodo_chat']:
            return (
                "<|im_start|>system\n"
...
class ClassificationSystem:
    ...
    def get_contextual_intent_directorat(self, processed_text):
        """Infer direktorat from context patterns, especially for common confusion pairs."""
        text = (processed_text or '').lower()
        if not text:
            return None

        pti_platform = [
            'lms', 'igracias', 'sirama', 'sso', 'toss', 'authenticator',
            'email telkom', 'one drive', 'onedrive', 'wifi',
        ]
        pti_issue = [
            'gabisa', 'gak bisa', 'tidak bisa', 'error', 'akses', 'login', 'masuk',
            'load', 'loading', 'verifikasi', 'kode', 'sinkron', 'cetak',
            'ga muncul', 'tidak muncul',
        ]

        kpa_program = [
            'msib', 'mbkm', 'magang', 'loker', 'karir', 'beasiswa', 'alumni',
            'osjur', 'oprec', 'kompetisi', 'aiesec', 'bangkit',
        ]
        kpa_service = ['ktm', 'pkkmb', 'sertif pkkmb', 'sertifikat pkkmb']

        akademik_terms = [
            'krs', 'sks', 'nilai', 'sidang', 'yudisium', 'matkul', 'mata kuliah',
            'semester', 'libur', 'wisuda', 'proposal', 'skripsi', 'tugas akhir',
            'prs', 'cumlaude', 'tak',
        ]

        sdm_terms = [
            'dosen', 'dosbing', 'dosbim', 'dospem', 'doswal', 'perwalian',
            'wali dosen', 'pegawai', 'kepegawaian', 'sdm',
        ]

        aset_terms = [
            'parkir', 'parkiran', 'gedung', 'fasilitas', 'ac', 'spanduk', 'asrama',
            'student center', 'kios', 'sewa', 'gate', 'tult', 'oplib', 'logistik',
        ]

        pasca_terms = [
            's2', 's3', 'pasca', 'pascasarjana', 'pasca sarjana', 'tpa',
            'fasttrack', 'advance learning',
        ]

        keuangan_terms = [
            'bpp', 'ukt', 'bayar', 'pembayaran', 'tagihan', 'biaya',
            'semesteran', 'cicilan', 'penundaan',
        ]

        # 1) Pasca is highly specific.
        if self._contains_any(text, pasca_terms):
            return ('Pasca Sarjana dan Advance Learning', 'context_pasca')

        # 2) PTI: platform/system issues or KRS/KSM technical workflow.
        if self._contains_any(text, pti_platform):
            if self._contains_any(text, pti_issue) or self._contains_any(text, ['krs', 'ksm', 'pra tracer', 'tracer']):
                return ('Pusat Teknologi Informasi', 'context_pti')

        if self._contains_any(text, ['krs', 'ksm']) and self._contains_any(
            text,
            ['cetak', 'sirama', 'igracias', 'gabisa', 'error', 'status', 'sinkron', 'siap registrasi'],
        ):
            return ('Pusat Teknologi Informasi', 'context_pti_krsksm')

        # 3) KPA: student programs and services.
        if self._contains_any(text, kpa_program):
            # Academic-conversion context should stay Akademik.
            if self._contains_any(text, ['sks', 'konversi', 'convert']) and self._contains_any(text, ['msib', 'mbkm', 'magang']):
                return ('Akademik', 'context_akademik_conversion')
            return ('Kemahasiswaan, Pengembangan Karir, Alumni', 'context_kpa_program')

        if 'ktm' in text and self._contains_any(
            text,
            ['hilang', 'ilang', 'digital', 'tap', 'tapping', 'ngurus', 'surat kehilangan', 'ambil ktm', 'cetak ktm'],
        ):
            return ('Kemahasiswaan, Pengembangan Karir, Alumni', 'context_kpa_ktm')

        if self._contains_any(text, kpa_service):
            return ('Kemahasiswaan, Pengembangan Karir, Alumni', 'context_kpa_service')

        # 4) SDM (dosen/perwalian interaction issues).
        if self._contains_any(text, sdm_terms):
            return ('Sumber Daya Manusia', 'context_sdm')

        # 5) Keuangan.
        if self._contains_any(text, keuangan_terms):
            return ('Keuangan', 'context_keuangan')

        # 6) Aset for physical facilities/logistics context.
        if self._contains_any(text, aset_terms):
            if 'wifi' in text and not self._contains_any(text, ['parkir', 'parkiran', 'gedung', 'ac']):
                return ('Pusat Teknologi Informasi', 'context_pti_wifi')
            return ('Aset & Sustainability', 'context_aset')

        # 7) Academic by explicit academic terms.
        if self._contains_any(text, akademik_terms):
            return ('Akademik', 'context_akademik')
    ...
    def get_stop_sequences(self, model_name):
        ...
        if 'komodo' in lowered and 'chat' in lowered:
            return base_stops + [
                "<|im_end|>",
                "<|im_start|>user",
                "<|im_start|>system",
    ...
    def build_compact_retry_prompt(self, text, model_name=None):
        ...
        if 'komodo' in lowered and 'chat' in lowered:
            return (
                "<|im_start|>system\n"
                "Anda adalah classifier direktorat Telkom University.\n"
                "Jawab tepat satu baris dengan format: Direktorat: [nama_direktorat], Keyword: [keyword_terkait]."
                "<|im_end|>\n"
                f"<|im_start|>user\n{text}<|im_end|>\n"
                "<|im_start|>assistant\n"
            )
        ...
        if any(name in lowered for name in ['llama', 'mistral', 'mixtral', 'komodo']):
            return (
                "[INST] Anda adalah classifier direktorat Telkom University. "
                ...
                "Kemahasiswaan, Pengembangan Karir, Alumni. "
                "Jawab tepat satu baris. Format wajib: Direktorat: [nama_direktorat], Keyword: [keyword_terkait]. "
                f"Teks: {text} [/INST]"
            )
        ...
        if 'qwen' in lowered:
            return (
                "<|im_start|>system\nAnda adalah classifier direktorat Telkom University. "
                "Pilih SATU direktorat paling relevan. Jawab tepat satu baris. "
                "Format wajib: Direktorat: [nama_direktorat], Keyword: [keyword_terkait]<|im_end|>\n"
                f"<|im_start|>user\nTeks: {text}<|im_end|>\n"
                "<|im_start|>assistant\n"
            )
    ...
    def process_single_text(self, text, llm, config_id, classification_mode='pure_llm', model_name=None):
        ...
        try:
            ...
            if not ('komodo' in (model_name or '').lower() and used_plain_fallback):
            ...
            is_komodo_chat = 'komodo' in lowered_model and 'chat' in lowered_model
            if is_komodo_chat and prompt_tokens > 300:
                self.log_message(
                    f"Komodo-chat prompt estimated at {prompt_tokens} tokens; using chatml compact first-pass prompt.",
                    "WARNING"
                )
                prompt = self.build_compact_retry_prompt(text, model_name)
                prompt_tokens = self.estimate_token_count(prompt)
            elif 'komodo' in lowered_model and prompt_tokens > 300:
                self.log_message(
                    f"Komodo prompt estimated at {prompt_tokens} tokens; using plain fallback first-pass prompt.",
                    "WARNING"
                )
                prompt = self.build_fallback_prompt(text)
                prompt_tokens = self.estimate_token_count(prompt)
            ...
            # Emit a lightweight resource snapshot before invoking the model
            try:
                snapshot = sample_resources()
                socketio = getattr(self, 'socketio', None)
                if socketio:
                    try:
                        socketio.emit('resource_update', snapshot, namespace='/')
                    except Exception as e:
                        print(f"Socket emit error (resource pre-invoke): {e}")
                else:
                    print("RESOURCE (pre-invoke):", snapshot)
            except Exception as e:
                print(f"Resource sampling error (pre-invoke): {e}")

            invoke_kwargs = {
                'max_tokens': CLASSIFY_MAX_OUTPUT_TOKENS,
                'temperature': CLASSIFY_TEMPERATURE,
            }
            if is_komodo_chat or CLASSIFY_USE_STOP_SEQUENCES:
                invoke_kwargs['stop'] = self.get_stop_sequences(model_name)

            try:
                start_invoke = time.time()
                output = llm.invoke(prompt, **invoke_kwargs)
                invoke_dur = time.time() - start_invoke
                if _PROM_AVAILABLE:
                    try:
                        LLM_INVOKE_LATENCY.observe(invoke_dur)
                        est_tokens = max(1, len(str(output)) // 4)
                        LLM_TOKENS_GENERATED.inc(est_tokens)
                    except Exception:
                        pass
            except Exception as e:
                if _PROM_AVAILABLE:
                    try:
                        LLM_INVOKE_ERRORS.inc()
                    except Exception:
                        pass
            ...
            if not str(output).strip():
                ...
                lowered_model = (model_name or '').lower()
                prefer_compact_retry = any(
                    name in lowered_model for name in ['mistral', 'mixtral', 'komodo', 'llama', 'llama2', 'llama3']
                )

                if strategy == 'db' and not prefer_compact_retry:
                    ...
                    # For INST/llama-family models and Komodo, the plain fallback prompt
                    # is more reliable than replaying the long DB prompt a second time.
                    if is_komodo_chat:
                        retry_prompt = self.build_compact_retry_prompt(text, model_name)
                    else:
                        retry_prompt = self.build_fallback_prompt(text)
                    retry_kwargs = {
                        'max_tokens': max(CLASSIFY_MAX_OUTPUT_TOKENS, 96),
                        'temperature': 0.0,
                    }
                    ...
                    if _PROM_AVAILABLE:
                        try:
                            LLM_TOKENS_GENERATED.inc(max(1, len(str(output)) // 4))
                        except Exception:
                            pass
                if strategy == 'off':
                    output = ""

                if not str(output).strip() and strategy != 'off':
                    # Final safeguard: compact prompt without stop sequences.
                    self.log_message(
                        "Empty output persists after retry. Using compact final fallback.",
                        "WARNING"
                    )
                    if is_komodo_chat:
                        retry_prompt = self.build_compact_retry_prompt(text, model_name)
            ...
            output = str(output)
            if is_komodo_chat:
                # Guard against chat-template control tokens leaking into text.
                output = re.split(r'<\|im_start\|>user|<\|im_start\|>system|<\|im_start\|>assistant', output)[0]
                output = output.replace('<|im_end|>', '')
            output = output.strip()
            print(f"📨 LLM Output: {output}")
            # Emit a snapshot after model invocation
            try:
                snapshot = sample_resources()
                socketio = getattr(self, 'socketio', None)
                if socketio:
                    try:
                        ...
                        print(f"Socket emit error (resource post-invoke): {e}")
                else:
                    print("RESOURCE (post-invoke):", snapshot)
            except Exception as e:
                print(f"Resource sampling error (post-invoke): {e}")
            # Update process-level gauges for Prometheus
            if _PROM_AVAILABLE:
                try:
                    PROC_CPU.set(psutil.cpu_percent(interval=None))
                    PROC_MEM_RSS.set(psutil.Process(os.getpid()).memory_info().rss)
                    if snapshot and snapshot.get('gpu_util') is not None:
                        GPU_UTIL.labels(gpu_index='0').set(snapshot.get('gpu_util'))
                except Exception:
                    pass
    ...
    def classify_universal(self, file_path, socketio=None, config_id=None, 
        ...
        # Keep socketio reference for lower-level emits (resource snapshots etc.)
        self.socketio = socketio
        # Start Prometheus server early so metrics are available during the run
        if _PROM_AVAILABLE and not getattr(self, '_prom_started', False):
            try:
                start_http_server(PROM_PORT)
                self._prom_started = True
                self.log_message(f"Prometheus metrics exposed on :{PROM_PORT}", socketio=socketio)
            except Exception as e:
                self.log_message(f"Failed to start Prometheus HTTP server: {e}", 'WARNING', socketio=socketio)
        ...
        if performance_mode is None:
            performance_mode = os.getenv('CLASSIFY_GPU_FIRST', '1') == '1'

        llm = init_llm_universal(model_name, ctx_size_override, performance_mode=performance_mode)
        
        # Checkpoint setup
        checkpoint_path = f"{os.path.splitext(file_path)[0]}_checkpoint.xlsx"
        if CLASSIFY_USE_CHECKPOINT:
            results, start_idx = self.load_checkpoint(checkpoint_path, len(df))
            if start_idx > 0:
                self.log_message(
                    f"Resuming from checkpoint at row {start_idx + 1}/{total_rows}: {checkpoint_path}",
                    socketio=socketio,
                )
        else:
            # Default behavior: always run a fresh full pass to avoid skipping rows
            # due to stale checkpoint files from previous runs.
            if os.path.exists(checkpoint_path):
                try:
                    os.remove(checkpoint_path)
                    self.log_message(
                        f"Stale checkpoint removed: {checkpoint_path}",
                        socketio=socketio,
                    )
                except Exception as e:
                    self.log_message(f"Could not remove checkpoint: {e}", "WARNING", socketio=socketio)
            results, start_idx = [None] * len(df), 0
        ...
        if _NVML_AVAILABLE:
            try:
                ...
                gpu = {
                    ...
                    'gpu_mem_used': int(getattr(meminfo, 'used', 0)),
                    'gpu_mem_total': int(getattr(meminfo, 'total', 0)),
                }
            except Exception:
                gpu = {}
        else:
            # fallback to nvidia-smi (best-effort)
            try:
                ...
                util, used, total = out.decode().strip().split(',')
                gpu = {'gpu_util': int(util.strip()), 'gpu_mem_used': int(used.strip()), 'gpu_mem_total': int(total.strip())}
            except Exception:
                gpu = {}
        ...
        for pattern in patterns:
            ...
            if matches:
                for match in matches:
                    ...
                    # Clean keyword: extract only first word before separators like . | , space followed by uppercase
                    # Extract only first word/short token as keyword
                    current_keyword = re.match(r'^(\S+)', current_keyword.strip())
                    current_keyword = current_keyword.group(1) if current_keyword else "-"
    ...
    if not direktorat:
        if IMPROVEMENTS_AVAILABLE:
            ...
            if found_dir in valid_direktorats:
                ...
                keyword = found_kw if re.search(r'\bkeyword\s*:', output, re.IGNORECASE) else "-"
        else:
            for valid_dir in valid_direktorats:
                if valid_dir.lower() in output.lower():
                    direktorat = valid_dir
                    keyword = extract_keyword_near_mention(output, valid_dir) if re.search(r'\bkeyword\s*:', output, re.IGNORECASE) else "-"
                    break
...
def init_llm_universal(model_name=None, ctx_size_override=None, performance_mode=None):
    ...
    if performance_mode:
        n_ctx = ctx_size_override or min(profile['optimal_context'] or 4096, 4096)
    ...
    # Reduce CPU threads in GPU mode to prevent CPU bottleneck
    if performance_mode:
        default_threads = max(2, os.cpu_count() // 4 if os.cpu_count() else 2)
    else:
        default_threads = max(4, min(8, os.cpu_count() or 8))
    
    gpu_layers_override = _env_int('CLASSIFY_GPU_LAYERS', 100)
    ...
    if is_windows and performance_mode:
        try:
            windows_gpu_layers = int(os.getenv('LLM_WINDOWS_GPU_LAYERS', '100'))
        except ValueError:
            windows_gpu_layers = 50
    ...
    if performance_mode:
        common_params['n_batch'] = 512          # Better GPU throughput while staying VRAM-safe
        common_params['f16_kv'] = True          # Keep KV cache in fp16 to favor GPU execution
        ...
        if n_ctx > 4096:
            common_params['n_ctx'] = 4096       # Cap context to 2K
    ...
    # NOTE: Setting n_gpu_layers higher than model total puts EVERYTHING on GPU
    if gpu_layers_override >= 0:
        ...
        # Use 100+ to force all layers to GPU (llama.cpp puts all if this exceeds model layers)
        if model_family in ['qwen', 'qwen2', 'qwen3', 'phi', 'gemma']:
            common_params['n_gpu_layers'] = 100  # Force all to GPU for small models
        elif model_family in ['llama', 'llama2', 'llama3']:
            common_params['n_gpu_layers'] = 100
        elif model_family in ['mistral', 'mixtral', 'komodo']:
            common_params['n_gpu_layers'] = 100  # Force all to GPU
        else:
            common_params['n_gpu_layers'] = 100
    else:
    ...
    if is_windows and performance_mode:
        ...
        raw_candidates = os.getenv('LLM_WINDOWS_GPU_LAYERS_CANDIDATES', '100,50,45,40,35,28,20,12')
        parsed_candidates = []
    ...
    safe_selected.update({
        'n_ctx': min(int(n_ctx), 4096 if performance_mode else 8192),
...
def classify_universal(file_path, socketio=None, config_id=None, model_name=None, 
                      ctx_size_override=None, performance_mode=None, benchmark_mode=False, strict_keyword_match=None,
                      classification_mode=CLASSIFY_DEFAULT_MODE):
